...
{\inred{In order to crystallize the comparison among  the core algorithms, 
...
We use  $k=1000$, {\inred{based on the assumption that simple tf-idf retrieval is the first phase of multi-stage ranking, 
...
{\inred{Finally, \pJASS\/ is instantiated with $p=0.01$ for low recall and $p=0.02$ for high recall (using $p=0.1$, as suggested, e.g., 
...
\bigdataset{  \cw & }
   97.5\%  &  98.5\%  & 98.5\%  & 99\%   & 97.5\% & 80\%  & \inred{TBD}
...
$95\%$ latencies of queries\bigdataset{ on  {\cw}}. The latter captures the so-called ``tail latency'' of 
