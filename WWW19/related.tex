...
\inred{
Mackenzie et al.~\cite{parallel-jass} have parallelized JASS, a score-order algorithm, and use different heuristics to determine when to stop traversing term lists and return the results. 
Like \alg, they consider approximate computaions. We show that \alg\ achieves better performance and better recall than this approach. This is thanks to a number of features we employ in \alg, 
including reducing the document map for better locality,  a fine-tuned stopping condition, and 
storing the top scores in a document heap instead of discovering them from the document map at the end of the traversal. 
}
