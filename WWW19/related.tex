...
it performs very little processing per-posting. In order to achieve early termination, JASS applies different heuristics that limit the fraction of processed terms. On the theoretical side, unlike TA, it provides no instance-optimality guarantee, and therefore, its exact variant is inefficient. Similarly to \alg, \pJASS\/ focuses on approximate top-k retrieval. We show that \alg\ achieves better performance and better recall, thanks to multiple optimizations -- e.g., continuous result-set pruning that leads to better locality, and a fine-tuned stopping condition.
