...
We present \emph{\alg}  -- \emph{Scalable PARallel Threshold Algorithm} -- for (approximate)
top-k retrieval on multi-core hardware. 
%\alg\ performs top-k retrieval much faster than the state-of-the-art, particularly for long queries and large search corpora. 
%Its
% tail
...
On a 500M-document index,  \alg's latency is virtually unchanged, whereas 
 RA takes 2 seconds and \inred{the latency of other existing parallel search algorithm surges even further. [Waiting for pJASS results on big data set.]}
