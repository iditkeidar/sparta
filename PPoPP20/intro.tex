...
Many Interactive big data environments need to select data based on an aggregate score consisting of multiple features; 
such queries are prevalent in information retrieval, web search, data mining, data analytics, and more~\cite{top-k-survey}. 
...
A query may retrieve the popular applications over a ten-day period by aggregating such access statistics from multiple lists.
Real-time analytics databases such as Apache Druid support TopN~\cite{druid-topN} queries in order to facilitate such queries.

% Problem: long queries
In modern use cases,  queries are becoming more complex and involve a larger number of features. 
Case in point - Internet users are changing the ways in which they interact with search engines. 
...
%Two stages
Data processing in these domains usually proceeds in two stages~\cite{Wang:2011}. 
...
The second stage performs more elaborate analysis, e.g., a web query engine 
re-ranks the  documents via some sophisticated (e.g., machine-learned) function, to produce the final result list (typically, a few tens of documents). 
...
stages can be satisfied by approximate results~\cite{Crane:2017,Lin:2015,Wang:2011,druid-topN}. 
...
%for retrieving the top-k objects from a database based on an aggregation of attributes that may reside in multiple nodes. 
%%%%Perhaps surprisingly, this algorithm was largely ignored in the web search literature.
%, despite its relevance. 
%In the web setting, term posting lists represent attributes and the ranking function is a linear aggregation of term scores. 
TA has two variants: (1) \emph{Random Access} ({\em RA}), which  
...
Section~\ref{sec:problem} defines the top-k retrieval problem and associated metrics, and 
